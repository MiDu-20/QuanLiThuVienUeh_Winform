...
namespace QuanLiThuVienUeh.nguoidung.game.Tetris
{
    internal class MapController
    {
        ...
        public static bool MovingLeft, MovingRight, MovingA, MovingSpace;
        //Method
        ...
        public static void DrawGrid(Graphics g)
        {
            for (int i = 0; i <= 16; i++)
            {
                g.DrawLine(new Pen(Color.FromArgb(226, 116, 75)), new Point(50, 50 + i * size), new Point(50 + 8 * size, 50 + i * size));
            }
            for (int i = 0; i <= 8; i++)
            {
                g.DrawLine(new Pen(Color.FromArgb(226, 116, 75)), new Point(50 + i * size, 50), new Point(50 + i * size, 50 + 16 * size));
            }
        }

        public static void MapPointing(System.Windows.Forms.Label label1, System.Windows.Forms.Label label2)
        {
            int count = 0;
        ...
        public static bool CheckCollideVertical()
        {
        ...
        public static bool CheckCollideHorizon(int LRIndex)
        {
            for (int i = currentShape.y; i < currentShape.y + currentShape.sizeMatrix; i++)
            {
                for (int j = currentShape.x; j < currentShape.x + currentShape.sizeMatrix; j++)
                {
                    if (currentShape.matrix[i - currentShape.y, j - currentShape.x] != 0)
                    {
                        if (j + 1 * LRIndex > 7 || j + 1 * LRIndex < 0)
                            return true;

                        if (map[i, j + 1 * LRIndex] != 0)
                        {
                            if (j - currentShape.x + 1 * LRIndex >= currentShape.sizeMatrix || j - currentShape.x + 1 * LRIndex < 0)
                            {
                                return true;
                            }
                            if (currentShape.matrix[i - currentShape.y, j - currentShape.x + 1 * LRIndex] == 0)
                                return true;
                        }
                    }
                }
